import { generateRandom } from "../../../../utils/functions/function"
...
import { updateBg } from "../../../../redux/actions/activity"
...
const Activity = ({ activity, date, bg, id }) => {
    ...
    return (
        <div
            ...
            ${!activityState.isGridView && 'w-full h-auto flex flex-col border-[1px] rounded-[4px] border-purple-300   '}
            rounded-[4px] border-[1px] border-purple-300 mb-[2rem] `}
        >

            {/* time and day */}
            <div style={{ zIndex: '10' }} className="z-5 sticky top-0 flex justify-between gap-[12px] bg-inherit text-purple-100 text-[20px] h-fit px-[10px] py-[10px] " >
                ...
                <>
                    {
                        activityState.filters.timeLapses
                            ...
                            <div className="  relative flex flex-col py-[6px]  " >
                                ...
                                {
                                    activity.map((timeLapse, index) => (
                                        <div style={{ borderBottom: '1px solid #ffffff4d', borderRadius: '0', gap: activityState.isGridView ? '8px' : '15px', padding: '4px 8px' }} key={index} className={`py-[1px] border-b-[1px] border-[#ffffff4d] rounded-0 flex items-start px-[8px] ${activityState.isGridView ? 'px-0 gap-[8px] ' : 'px-[8px] gap-[15px] '} text-purple-100 rounded-[4px] `} >
                            ...
                            <div id="non_timeLapses_activity" className=" flex flex-col gap-[4px]  " >
                                {
                                    activity.map((timeLapse, index) => (
                                        ...
                                        <span key={index} style={{ display: 'inline-block', minWidth: '46%' }} className="relative min-h-[20px] min-w-[46%] sm:mr-[20px] mr-0 " >
                                            ...
                                            {activityState.filters.text && <p style={{ paddingLeft: '15px' }} className="pl-[15px] text-purple-300 " >{timeLapse.text}</p>}
                                        </span>
                                    ))
                                }
                            </div>
                        ...
                        </Tooltip>
                        {
                            ...
                            <div style={{ boxShadow: '1px 2px 6px 3px rgb(0 0 0 / 50%)', top: '4px', right: '40px' }} className="absolute top-[4px] right-[40px] flex justify-between  rounded-[8px] p-[2px] " >
                                <Tooltip title="Default" placement="bottom" >
                                    <DoNotDisturb onClick={() => handleSetColor('purple-900', '202124')} style={{ fontSize: '25px' }} className={` ${background.color == 'purple-900' && 'border-1px border-white'} cursor-pointer text-[#a4a2a5]  text-[40px] `} />
                                </Tooltip>
