...
import { bookmarkNote } from "../../actions/note"
import { useStateContext } from '../../contexts/ContextProvider';
import { limitText, Capitalize, LightenDarkenColor } from '../../utilityFunctions/function';
...
const Note = ({ noteObj, id, uploadedImages, note, tags, type, createdAt, updatedAt, bg, bookmarked }) => {
    ...
    useEffect(() => {
        console.log('noteData', noteState.noteData)
    }, [noteState.noteData])
        ...
        <>
            ...
            }
            {
                <div
                    ...
                    className={` min-w-[250px] max-w-[250px] min-h-[20rem] max-h-[20rem] ${bg?.hex ? `bg-${bg?.color}` : 'bg-gray-900'} p-4 mb-4 rounded-[1rem] `}
                >
                        ...
                        <button onClick={() => handleStarClick(id)} style={{ background: 'black', width: '30px', height: '30px' }} className=" bg-black rounded-full w-[30px] h-[30px] flex justify-center items-center " >
                            {
                                bookmarked
                                    ?
                                    <Star style={{ fontSize: '16px' }} className="text-gray-300 text-[16px] " />
                                    :
                                    <StarOutline style={{ fontSize: '16px' }} className="text-gray-300 text-[16px] " />
                            }
                        </button>
