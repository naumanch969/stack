...
const NoteForm = ({ updateTheNote, updateNoteId }) => {
    ...
    const Tag = ({ title }) => (
        <div style={{ background: noteState.noteData.bg?.hex ? `#${noteState.noteData.bg.hex}` : `#202124`, width: 'fit-content', height: 'fit-content' }} className="flex w-fit h-fit gap-2 items-center justify-between rounded-[15px] py-[3px] px-[7px]  " >
            ...
            <Cancel style={{ background: noteState.noteData.bg?.hex ? `#${noteState.noteData.bg.hex}` : `#202124`, fontSize: '1rem' }} onClick={() => handleFilterTag(title)} className={`cursor-pointer text-black text-[1rem] bg-gray-900  rounded-full `} />
        </div>
    )
    ...
    return (
        ...
        <div style={{ background: noteState.noteData.bg?.hex ? `#${noteState.noteData.bg.hex}` : `#202124`, borderRadius: '1rem' }} className={`relative flex flex-col justify-between rounded-4 min-w-[32rem] max-h-[32rem] h-[19rem]  bg-${noteState.noteData.bg.color ? noteState.noteData.bg : 'gray-900'}  p-4 mb-4`} >
            {/* close button of note form */}
            <Cancel onClick={handleCloseNoteForm} style={{ background: noteState.noteData.bg?.hex ? `#${noteState.noteData.bg.hex}` : `#202124` }} className={`absolute top-[-5px] right-[-5px] cursor-pointer bg-${noteState.noteData.bg?.color} text-gray-300 rounded-full `} />
            ...
            <div style={{ height: '90%' }} className="flex justify-between items-center gap-4 h-[90%] overflow-y-hidden overflow-x-hidden " >
                ...
                <div style={{ height: '100%', width: '14.5rem' }} className=" flex flex-col gap-2 relative h-full w-[14.5rem] " >
                    {/* type input */}
                    <div style={{ height: '12%', borderRadius: '5px' }} className="bg-gray-300  px-[5px] py-[3px] h-[12%] relative flex justify-center gap-4 rounded-[5px] " >
                    ...
                    <div style={{ height: "88%", borderRadius: '5px' }} className="relative w-full h-[88%] " >
                        <textarea
                            ...
                            className="code_highlight resize-none w-full h-full bg-gray-300 border-none overflow-x-scroll border-[1px] p-[14px] rounded-[5px] placeholder-gray-400 bg-transparent border-gray-400 text-gray-400 focus:outline-none "
                            autoComplete='off'
                            name="note"
                            placeholder="Note"
                ...
                <div style={{ width: '14.5rem', height: '100%' }} className="flex flex-col gap-2 w-[14.5rem] h-full " >
                    ...
                    {/* tag input */}
                    <div style={{ height: '44%' }} className="relative p-[7px] bg-gray-300 flex flex-wrap gap-4 w-full max-h-[50%] h-[44%] min-h-8 rounded-[5px]  overflow-y-scroll" >
                        <input
                    ...
                    {/* image file input */}
                    <div style={{ height: '44%' }} className="relative p-[7px]  bg-gray-300 max-h-[50%] h-[44%] min-h-8 rounded-[5px] overflow-y-scroll overflow-x-hidden " >
                    ...
                    <div style={{ height: '12%', borderRadius: '5px' }} className="h-[12%] rounded-[5px] " >
                        <select className="w-full h-full rounded-[5px] outline-none cursor-pointer " onChange={(e) => setNoteState({ ...noteState, noteData: { ...noteState.noteData, type: e.target.value } })} >
                            <option className="checked:bg-gray-900 checked:text-gray-300 " value="general" >General</option>
                            <option className="checked:bg-gray-900 checked:text-gray-300 " value="work" >Work</option>
                            <option className="checked:bg-gray-900 checked:text-gray-300 " value="personal" >Personal</option>
                            <option className="checked:bg-gray-900 checked:text-gray-300 " value="project" >project</option>
                        </select>
                    </div>
            ...
            <div style={{ height: '10%', position: 'relative', top: '7px' }} className='flex justify-between relative top-[7px] overflow-y-initial ' >
                ...
                <Tooltip title={updateTheNote ? 'Update' : 'Create'} placement="top" >
                    <button onClick={updateTheNote ? handleUpdate : handleCreate} style={{ background: 'black', width: '30px', height: '30px' }} className=" bg-black rounded-full w-[30px] h-[30px] flex justify-center items-center " >
                        <Done style={{ fontSize: '16px' }} className="text-gray-300 text-[16px] " />
                    </button>
                </Tooltip>
            </div>
