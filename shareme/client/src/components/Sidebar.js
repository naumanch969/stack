...
const isNotActiveStyle = 'flex items-center px-5 text-gray-500 hover:text-black transition-all duration-200 gap-3 ease-in-out capitalize '
const isActiveStyle = 'flex items-center px-5 font-extrabold border-r-2 border-black   transition-all duration-200 gap-3 ease-in-out capitalize '
const categories = [
    { name: 'Animals' },
    { name: 'Gaming' },
    { name: 'Coding' },
                ...
                <div className="flex flex-col gap-5" >
                    ...
                    <h3 className="mt-2 px-5 text-base 2xl:text-xl" >Discover Categories</h3>
                    {
                        categories.slice(0, categories.length - 1).map((category) => (
                            <NavLink key={category.name} onClick={handleCloseSidebar} to={`/category/${category.name}`}  >
                                {category.name}
                            </NavLink>
                        ))
                    }
